...
urlpatterns = [
    ...
    # url(r'^assignments/(?P<pk>\d+)/$', AssignmentDetailView.as_view(),
    #       name='aAssignmentSessionResulthViewssignment_deteil'),
    # url(r'^assignments/(?P<slug>[\w-]+)/final/$', views.AssignmentSessionFinalView.as_view(),
    ...
    url(r'^assignment-results/(?P<slug>[\w-]+)/$', views.AssignmentResultDetailView.as_view(),
        name='assignment_result_detail'),

    url(r'^assignment-users-results/$', views.CuratorAssignmentList.as_view(), name='supper_assignments_list'),
    url(r'^assignment-users-results/(?P<slug>[\w-]+)/$', views.CuratorAssignmentSessionList.as_view(),
        name='supper_assignments_session_list'),
    url(r'^assignment-users-results/(?P<slug>[\w-]+)/(?P<pk>\d+)/$',
        views.CuratorAssignmentUserSessionResult.as_view(),
        name='supper_sesion_result_detail'),
    ...
    url(r'^subjects/(?P<slug>[\w-]+)/topic/(?P<topic_slug>[\w-]+)/$', views.TopicDetailView.as_view(),
        name='topic_detail'),
    url(r'^subjects/(?P<slug>[\w-]+)/topic/(?P<topic_slug>[\w-]+)/check/$', views.TopicCheckAnswerView.as_view(),
        name='topic_check'),
    # url(r'^subjects/(?P<slug>[\w-]+)/topic/(?P<topic_slug>[\w-]+)/check_user_answer/$',
    #       views.check_user_answer, name='check_user_answer'),
