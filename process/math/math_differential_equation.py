...
def generate_diff_eq():
    ...
    questions = [Eq(n1*y(x).diff(x, x) + n2*y(x).diff(x) + n3*y(x), 0),
...
def null_answer_diff_eq(expression, answer):
    if str(solve_diff_eq(expression)) == str(answer):
        return True
    else:
        return False